<div
 ...
 max-width="screen-lg"
>
 <h2 class="mb-8 text-xl font-bold text-gray-700">SKJ {{ $occupation->name }} {{ $occupation->nomenclature }}</h2>
 <div class="grid grid-cols-1 gap-6 @xl:grid-cols-2 @2xl:grid-cols-3">
  @foreach ($occupation->competencyStandards as $competency)
   @php
    $path = route('files.show', [
        'disk' => $competency->file_disk,
        'path' => $competency->file_path,
    ]);
   @endphp
   <div class="group relative flex gap-2 overflow-hidden rounded-md border bg-white p-2">
    ...
    <div class="flex h-full w-full flex-col gap-1">
     <h3 class="min-h-10 line-clamp-2 text-base font-semibold leading-5 text-gray-800">{{ $competency->description }}
     </h3>
     <p class="mb-4 text-xs font-semibold text-orange-500">
      {{ $competency->created_at->format('d F Y') }}
     </p>
     <div
     ...
     >
      <a
       href="{{ $path }}"
       download="{{ $competency->description }}"
