...
{
  public string $tableName = 'organization-table';
      ...
      ->add('status_template', function ($row) {
        return [
          'statusTemplate' => [
            ...
            'verifyEvent' => 'verify-organization',
          ]
        ];
      })
      ->add('year')
  ...
  #[\Livewire\Attributes\On('verify-organization')]
  public function verifyRow($rowId): void
  {
    $this->dispatch(
      event: 'show-modal',
      component: 'verify-organization-form',
  ...
  {
    try {
      $organization->update(compact('status', 'description'));
