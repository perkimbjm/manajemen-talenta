 ...
 <article class="card card-compact bg-white @container">
  <div class="card-body">
   <div class="mb-2 flex gap-4 border-b">
    <h3 class="px-4 pb-2 text-left text-xl font-semibold">Tugas Pendukung / Tambahan</h3>
    <div class="ml-auto">
     <button
      class="btn btn-warning btn-sm"
      x-on:click="()=> {
        $wire.updateAssessmentExtra()
       }"
     >
      Update Rekap
     </button>
     <button
      ...
      x-on:click="()=> {
        $dispatch('show-modal', {
          component: 'supporting-task-form',
          title: 'Tambah Tugas Pendukung / Tambahan',
          arguments: {
   ...
   <div class="overflow-x-auto">
    <table class="table">
     ...
     <tbody>
      @foreach ($supportingTasks as $supportingTask)
       <tr
        x-id="['row']"
        wire:key="{{ $supportingTask->id }}"
       >
        <th>
         <label>
          <input
        ...
        <td>{{ $supportingTask->scope }}</td>
        <td>
         {{ $supportingTask->description }}
        </td>
        <td>
         <div class="grid gap-2">
          @if ($user->hasExactRoles('Pegawai'))
           @foreach (['Surat Keputusan (SK)', 'Surat Keterangan'] as $type)
            <button
             class="btn btn-sm w-48 justify-start text-left"
             x-on:click="$dispatch('show-modal', {
          component: 'supportingTask-document-form',
          title: 'Berkas Tugas Pendukung [{{ $type }}',
          arguments: {
            supportingTask: '{{ $supportingTask->id }}',
            type: '{{ $type }}',
          }
        })"
            >
             ...
             @if ($supportingTask->getMedia($type)->count() > 0)
          ...
          @else
           @php
            $documents = $supportingTask->getMedia('*');
           @endphp
        ...
        <td>
         @can('Ubah Status Penilaian')
          ...
          >
           <div class="grid gap-2">
            <button
             @disabled($supportingTask->status >= 1)
             class="btn btn-ghost btn-sm justify-start text-left"
             x-on:click="$dispatch('confirm-modal', {
              content: 'Apakah anda yakin akan menolak tugas Pendukung ini?',
              prompt: true,
              promptPlaceholder: 'Alasan Penolakan',
              disableConfirm: (promptInput) => promptInput === '',
              confirmAction: (promptInput) => $wire.changeStatus('{{ $supportingTask->id }}', 1, promptInput)
             })"
            >Ditolak</button>
            <button
             @disabled($supportingTask->status >= 2)
             class="btn btn-ghost btn-sm justify-start text-left"
             x-on:click="$dispatch('show-modal', {
              component: 'supporting-task-verify-form',
