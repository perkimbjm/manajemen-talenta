  ...
  <div class="card-body">
   <div class="overflow-x-auto">
    <div class="mb-2 flex gap-4 border-b">
     <h3 class="px-4 pb-2 text-left text-xl font-semibold">Partisipasi Tim/Pokja</h3>
     <div class="ml-auto">
      <button
       class="btn btn-warning btn-sm"
       x-on:click="()=> {
        $wire.updateAssessmentOrganization()
       }"
      >
       Update Rekap Tim/Pokja
      </button>
      <button
       ...
       x-on:click="()=> {
        $dispatch('show-modal', {
          ...
          title: 'Tambah Partisipasi Tim/Pokja',
          arguments: {
        ...
        <td>
         <div class="grid gap-2">
          @if ($user->hasExactRoles('Pegawai'))
           @foreach (['SK Tim/Pokja', 'Surat Keterangan'] as $type)
            <button
             class="btn btn-sm w-48 justify-start text-left"
             x-on:click="$dispatch('show-modal', {
          component: 'organization-document-form',
          title: 'Berkas Organisasi [{{ $type }}',
          arguments: {
            ...
            >
             ...
             @if ($organization->getMedia($type)->count() > 0)
          ...
          @else
           @php
            $documents = $organization->getMedia('*');
           @endphp
           ...
           <div class="grid gap-2">
            <button
             ...
             x-on:click="$dispatch('confirm-modal', {
              content: 'Apakah anda yakin akan menolak organisasi ini?',
              prompt: true,
              promptPlaceholder: 'Alasan Penolakan',
              disableConfirm: (promptInput) => promptInput === '',
              confirmAction: (promptInput) => $wire.changeStatus('{{ $organization->id }}', 1, promptInput)
             })"
            >Ditolak</button>
            <button
             ...
             class="btn btn-ghost btn-sm justify-start text-left"
             x-on:click="$dispatch('show-modal', {
              component: 'organization-verify-form',
